...
lemma derivAtTop_add' (hf_cvx : ConvexOn ℝ (Ici 0) f) (hg_cvx : ConvexOn ℝ (Ici 0) g) :
  ...
  suffices Tendsto (fun x ↦ (rightDeriv f x : EReal) + ↑(rightDeriv g x)) atTop
    ...
    rw [EventuallyEq, eventually_atTop]
    refine ⟨1, fun x hx ↦ ?_⟩
    rw [show (rightDeriv (f + g) x : EReal) = ((rightDeriv f x + rightDeriv g x : ℝ) : EReal) from
      congrArg _ (rightDeriv_add_apply
        (hf_cvx.differentiableWithinAt_Ioi' (zero_lt_one.trans_le hx))
...
lemma slope_le_rightDeriv (h_cvx : ConvexOn ℝ (Ici 0) f) {x y : ℝ} (hx : 0 ≤ x) (hxy : x < y) :
  ...
  refine le_csInf (Set.Nonempty.image _ ⟨y + 1, by simp⟩) (fun b hb ↦ ?_)
  obtain ⟨z, hyz, rfl⟩ := hb
