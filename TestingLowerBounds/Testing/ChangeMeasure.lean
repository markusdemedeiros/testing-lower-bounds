...
lemma setLIntegral_nnnorm_exp_neg_llr_le [SigmaFinite ν] [SigmaFinite μ]
  ...
  calc ∫⁻ a in s, ‖rexp (-llr μ ν a)‖₊ ∂μ
    ...
    _ = ∫⁻ a in t, ν.rnDeriv μ a ∂μ := by
        ...
        rw [show (‖(ν.rnDeriv μ x).toReal‖₊ : ℝ≥0∞) = ‖(ν.rnDeriv μ x).toReal‖ₑ from rfl,
          Real.enorm_of_nonneg ENNReal.toReal_nonneg, ENNReal.ofReal_toReal hx]
...
lemma measure_sub_le_measure_mul_exp [SigmaFinite μ] [IsFiniteMeasure ν] (hμν : μ ≪ ν)
  ...
  _ = ∫ _ in s ∩ {x | llr μ ν x ≤ c}, exp (- c) ∂μ := by
        rw [setIntegral_const _, smul_eq_mul, Measure.real_def]
  _ ≤ ∫ x in s ∩ {x | llr μ ν x ≤ c}, exp (- llr μ ν x) ∂μ := by
        refine setIntegral_mono_ae_restrict ?_ ?_ ?_
        · exact integrableOn_const ((measure_mono Set.inter_subset_left).trans_lt (Ne.lt_top hμs)).ne
